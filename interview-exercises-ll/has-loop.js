...
class LinkedList {
    ...
    // Solution from teacher: no need for else branch, every other case is false, 
    // so return false outside if and only deal with loop check inside if
    // every other case: empty, 1 item, no loop
    /*
    hasLoop() {
        let slow = this.head;
        let fast = this.head;
